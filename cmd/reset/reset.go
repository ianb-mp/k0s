...
func (c *command) reset() error {
	...
	if nodeCfg.Spec.Storage.Kine != nil && nodeCfg.Spec.Storage.Kine.DataSource != "" {
		logrus.Warn("Kine dataSource is configured. k0s will not reset the data source. If you plan to continue using the data source, you should reset it to avoid conflicts.")
	}
